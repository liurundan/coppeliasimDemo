...
MultiPacketUDPCommunication::MultiPacketUDPCommunication () noexcept : receivePacketStatistics (5000), sendPacketStatistics (5000) {
...
MultiPacketUDPCommunication::~MultiPacketUDPCommunication () noexcept {
  for (unsigned int i=0; i<receivedBuffer.size(); i++)
    for (unsigned int j=0; j<receivedBuffer[i].size(); j++)
      delete [] receivedBuffer[i][j];
}

bool MultiPacketUDPCommunication::init_as_server (int port) noexcept {
  return socket.init_as_server (port);
}

bool MultiPacketUDPCommunication::init_as_client (const char* host, int port) noexcept {
  bool success = socket.init_as_client (host, port);
  partnerAddressMulti = socket.partner_address();
  return success;
}

void MultiPacketUDPCommunication::close () noexcept {
  socket.close ();
}

unsigned int MultiPacketUDPCommunication::send (const std::vector<char*> buffers, const std::vector<unsigned int> buflens) noexcept {
...
unsigned int MultiPacketUDPCommunication::sendto (const struct sockaddr_in& add, const std::vector<char*> buffers, const std::vector<unsigned int> buflens) noexcept {
  partnerAddressMulti = add;
  return send (buffers, buflens);
}

void MultiPacketUDPCommunication::receive_all (unsigned int max_buf_len)  {
...
unsigned int MultiPacketUDPCommunication::receivefrom (const struct sockaddr_in& add, std::vector<char*>& buffers, std::vector<unsigned int>& buflens, unsigned int max_buf_len)  {
...
unsigned int MultiPacketUDPCommunication::receive (std::vector<char*>& buffers, std::vector<unsigned int>& buflens, unsigned int max_buf_len)  {
  receive_all (max_buf_len);
  buffers.clear();
  buflens.clear();
...
const struct sockaddr_in& MultiPacketUDPCommunication::partner_address () const noexcept {
  return partnerAddressMulti;
}

bool MultiPacketUDPCommunication::started() const noexcept {
  return socket.started();
}

bool MultiPacketUDPCommunication::as_server() const noexcept {
  return socket.as_server();
}

UDPCommunicationStatistics MultiPacketUDPCommunication::send_load (int dt) const noexcept {
...
UDPCommunicationStatistics MultiPacketUDPCommunication::receive_load (int dt) const noexcept {
  UDPCommunicationStatistics ret;
  ret.packet_rate=0;
  ret.packet_size=0;
  Time from;
