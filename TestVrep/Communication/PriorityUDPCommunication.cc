...
PriorityUDPCommunication::PriorityUDPCommunication (const char* mb, unsigned int ml)  : max_buffer_size (ml+2), send_buffers (0), send_buffer_length (0), send_buffer_priorities (0), receive_buffers (0), receive_buffer_length (0), get_buffer (0), get_message (0) {
  magic_byte[0]=mb[0];
  magic_byte[1]=mb[1];
}


PriorityUDPCommunication::~PriorityUDPCommunication () noexcept {
  clear_send_buffer ();
  clear_receive_buffer ();
}


void PriorityUDPCommunication::clear_receive_buffer () noexcept {
...
void PriorityUDPCommunication::clear_send_buffer () noexcept {
...
unsigned int PriorityUDPCommunication::max_message_length () const noexcept {
  return max_buffer_size-4;
}


unsigned int PriorityUDPCommunication::send () noexcept {
...
unsigned int PriorityUDPCommunication::sendto (const struct sockaddr_in& add) noexcept {
...
unsigned int PriorityUDPCommunication::receive ()  {
  clear_receive_buffer ();
...
unsigned int PriorityUDPCommunication::receivefrom (const struct sockaddr_in& add)  {
  clear_receive_buffer ();
...
bool PriorityUDPCommunication::put (const char* msg, unsigned int msg_len, unsigned int prio)  {
...
bool PriorityUDPCommunication::get (const char*& mb, unsigned int& ml) noexcept {
...
bool PriorityUDPCommunication::init_as_server (int port) noexcept {
  return socket.init_as_server (port);
}

bool PriorityUDPCommunication::init_as_client (const char* host, int port) noexcept {
  return socket.init_as_client (host, port);
}

void PriorityUDPCommunication::close () noexcept {
  return socket.close();
}

const struct sockaddr_in& PriorityUDPCommunication::partner_address () const noexcept {
  return socket.partner_address ();
}

bool PriorityUDPCommunication::started() const noexcept {
  return socket.started();
}

bool PriorityUDPCommunication::as_server() const noexcept {
  return socket.as_server();
}

UDPCommunicationStatistics PriorityUDPCommunication::send_load (int dt) const noexcept {
  return socket.send_load(dt);
}

UDPCommunicationStatistics PriorityUDPCommunication::receive_load (int dt) const noexcept {
